...
fixid_z_ = torch.randn((5*5, 100)).view(-1, 100, 1, 1)
fixid_z_ = Variable(fixid_z_.cuda(), volatile=True)
...
#
#if (temp.shape != img_size) or (temp.shape[0] != img_size):
...
if not os.path.isdir('dcgan_results/random'):
    os.mkdir('dcgan_results/random')
if not os.path.isdir('dcgan_results/fixed'):
    os.mkdir('dcgan_results/fixed')
...
#            x_ = x_[:, :, 22:86, 22:86]
    ...
    p = 'dcgan_results/random/dcgan_' + str(epoch + 1) + '.png'
    fixed_p = 'dcgan_results/fixed/dcgan_' + str(epoch + 1) + '.png'
...
for e in range(train_epoch):
    img_name = 'dcgan_results/fixed/dcgan_' + str(e+1) + '.png'
